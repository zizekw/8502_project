...
#ifelse(glm_data=="yes", 1, 0)
glm_data$y = factor(glm_data$y, levels = c("yes", "no"))
#glm_data = subset(glm_data, select = -c('emp.var.rate'))
glm_data = glm_data[,-c(8:12)]

# create test data
# random select 24713 rows as training data, the rest rows are testing data
smp_size = floor(0.5 * nrow(glm_data))
